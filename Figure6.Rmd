...
### Figure 6B = antiviral responses pathway heatmap
...
### Figure 6C = T cell dysfunction pathway heatmap
...
###### added on jan 14 for exhaustion gene list
...
pdf(file="../plots/fig6_exhaus_prog_term.pdf",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()

svg(file="../plots/fig6_exhaus_prog_term.svg",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()

write.csv(df_meta, '../result/df_meta_scores_additional_exh_prog_term_jan24.csv')
```
...
sen_t_genes = c(sen_t_genes, gmt_file_cgp$genesets[gmt_file_cgp$geneset.names == 'REACTOME_SENESCENCE_ASSOCIATED_SECRETORY_PHENOTYPE_SASP'])
sen_t_genes = unlist(sen_t_genes)

result = df_meta
##### start calc

targetScore=gsva(t(df),list(ex_t_genes),method="ssgsea",ssgsea.norm=TRUE,verbose=T)
result['Exhaustion_tcell'] = t(targetScore)
df_meta['Exhaustion_tcell'] = t(targetScore)

targetScore=gsva(t(df),list(ex_non_t_genes),method="ssgsea",ssgsea.norm=TRUE,verbose=T)
result['Exhaustion_nontcell'] = t(targetScore)
df_meta['Exhaustion_nontcell'] = t(targetScore)

targetScore=gsva(t(df),list(sen_t_genes),method="ssgsea",ssgsea.norm=TRUE,verbose=T)
result['Senescence_tcell'] = t(targetScore)
df_meta['Senescence_tcell'] = t(targetScore)
...
pdf(file="../plots/fig6_exhaus.pdf",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()

svg(file="../plots/fig6_exhaus.svg",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()

write.csv(df_meta, '../result/df_meta_scores_additional_exh_jan24.csv')
```
...
library(readxl)
targetsheet = readxl::read_excel('../data/Fig_6_genes_jan2024.xlsx', sheet = 'Ag_presentation')
...
for (i in c(1:n)){
  ...
  source = targetsheet[i,'Source']
  if (source == 'Manual'){ # manual count genes
    genlist = unlist(str_split(targetsheet[i,'Genes'], '/'))
    genlist = gsub('\\.','-', genlist)
    print(genlist)
    
    # in this case we calculated by manual gene lists
    name = as.character(targetsheet[i,'Pathway'])
    ...
    pathway = as.character(targetsheet[i,'Pathway'])
    gmt_file_match = unlist(gmt_file$genesets[gmt_file$geneset.names ==  pathway])
    gmt_file_match_im = unlist(gmt_file_imm$genesets[gmt_file_imm$geneset.names ==  pathway]) 
    genlist = c(gmt_file_match, gmt_file_match_im)
    genlist = gsub('\\.','-', genlist)
    print(genlist)
    
    targetScore=gsva(t(df),list(genlist),method="ssgsea",ssgsea.norm=TRUE,verbose=T)
    result[pathway] = t(targetScore)
    df_meta[pathway] = t(targetScore)
...
#plotdf = plotdf[c('pos_CD4mem', 'neg_CD4mem', 'pos_CD4naive', 'neg_CD4naive',
#                  'pos_CD8mem', 'neg_CD8mem', 'pos_CD8naive', 'neg_CD8naive',
#                  'pos_Other', 'neg_Other', 'pos_Tumor', 'neg_Tumor'),]
...
## read all input targets
library(readxl)
targetsheet = readxl::read_excel('../data/Fig_6_genes_jan2024.xlsx', sheet = 'T_cytotoxic')
...
for (i in c(1:n)){
  ...
  if (source == 'Manual'){ # manual count genes
    ...
    gmt_file_match_im = unlist(gmt_file_imm$genesets[gmt_file_imm$geneset.names ==  pathway]) 
    genlist = c(gmt_file_match, gmt_file_match_im)
    # incase it is in immune
    #print(c(length(genlist), pathway))
...
pdf(file="../plots/fig6_updated_T_cytotoxic_jan2024.pdf",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()

svg(file="../plots/fig6_updated_T_cytotoxic_jan2024.svg",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()
...
## read all input targets
library(readxl)
targetsheet = readxl::read_excel('../data/Fig_6_genes_v3.xlsx', sheet = 'T_dysregulation_T')
...
for (i in c(1:n)){
  ...
  if (source == 'Manual'){ # manual count genes
    ...
    gmt_file_match = unlist(gmt_file$genesets[gmt_file$geneset.names ==  pathway])
    gmt_file_match_im = unlist(gmt_file_imm$genesets[gmt_file_imm$geneset.names ==  pathway]) 
    gmt_file_match_cgp = unlist(gmt_file_cgp$genesets[gmt_file_cgp$geneset.names ==  pathway]) 
    genlist = c(gmt_file_match, gmt_file_match_im, gmt_file_match_cgp)
    # incase it is in immune
    #print(c(length(genlist), pathway))
...
pdf(file="../plots/fig6_updated_T_dysregulation_T_jan24.pdf",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()

svg(file="../plots/fig6_updated_T_dysregulation_T_jan24.svg",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()
...
## read all input targets
library(readxl)
targetsheet = readxl::read_excel('../data/Fig_6_genes_v3.xlsx', sheet = 'T_dysregulation_non_T')
...
pdf(file="../plots/fig6_updated_T_dysregulation_non_T_jan24.pdf",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()

svg(file="../plots/fig6_updated_T_dysregulation_non_T_jan24.svg",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()
...
## read all input targets
library(readxl)
targetsheet = readxl::read_excel('../data/Fig_6_genes.xlsx', sheet = 'TME_remodeling')
...
pdf(file="../plots/fig6_updated_TME_remodeling_jan24.pdf",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()

svg(file="../plots/fig6_updated_TME_remodeling_jan24.svg",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()
...
targetlist = c(
  ...
  'MHC_CLASS_II_BIOSYNTHETIC_PROCESS',
  'MHC_CLASS_II_PEPTIDE_LOADING_COMPLEX ',
  'MHC_CLASS_II_PROTEIN_COMPLEX',
  'MHC_CLASS_II_RECEPTOR_ACTIVITY',
  'MHC_CLASS_II_PROTEIN_COMPLEX_BINDING',
  'MHC_CLASS_II_PROTEIN_BINDING'
...
for (i in c(1:n)){
  ...
  pathway =  targetlist[i]
  id = grep(pathway,gmt_file$geneset.names)[1]
  if (!is.na(id)){
    gmt_file_match = unlist(gmt_file$genesets[id])
    genlist = gmt_file_match
    genlist = gsub('\\.','-', genlist)
    targetScore=gsva(t(df),list(genlist),method="ssgsea",ssgsea.norm=TRUE,verbose=T)
    result[pathway] = t(targetScore)
    df_meta[pathway] = t(targetScore)
...
pdf(file="../plots/fig6_updated_newgenelist_MHCII_jan24.pdf",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()

svg(file="../plots/fig6_updated_newgenelist_MHCII_jan24.svg",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()
...
targetlist = c(
  ...
  'GOBP_REGULATION_OF_T_CELL_COSTIMULATION',
...
for (i in c(1:n)){
 ...
 # this case is msigdb
    ...
    #gmt_file_match = gmt_file$genesets[str_detect(gmt_file$geneset.names, 'ANGIOGENESIS')]
  pathway =  targetlist[i]#as.character(targetsheet[i,'Pathway'])
  gmt_file_match = unlist(gmt_file$genesets[gmt_file$geneset.names ==  pathway])
  gmt_file_match_im = unlist(gmt_file_imm$genesets[gmt_file_imm$geneset.names ==  pathway]) 
  gmt_file_match_cgp = unlist(gmt_file_cgp$genesets[gmt_file_cgp$geneset.names ==  pathway]) 
  gmt_file_match_hall = unlist(gmt_file_hall$genesets[gmt_file_hall$geneset.names ==  pathway]) 
...
pdf(file="../plots/fig6_updated_newgenelist_patch3_jan24.pdf",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()

svg(file="../plots/fig6_updated_newgenelist_patch3_jan24.svg",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()
...
targetlist = c(
  ...
  'GOBP_NEGATIVE_REGULATION_OF_CHRONIC_INFLAMMATORY_RESPONSE',
  'GOBP_REGULATION_OF_CHRONIC_INFLAMMATORY_RESPONSE',
  'HP_CHRONIC_INFECTION',
  'HALLMARK_INFLAMMATORY_RESPONSE'
...
for (i in c(1:n)){
 ...
 # this case is msigdb
  ...
  gmt_file_match_cgp = unlist(gmt_file_cgp$genesets[gmt_file_cgp$geneset.names ==  pathway]) 
  gmt_file_match_hall = unlist(gmt_file_hall$genesets[gmt_file_hall$geneset.names ==  pathway])
  gmt_file_match_hp = unlist(gmt_file_hp$genesets[gmt_file_hp$geneset.names ==  pathway])

  genlist = c(gmt_file_match, gmt_file_match_im, gmt_file_match_cgp, gmt_file_match_hall, gmt_file_match_hp)
    # incase it is in immune
    #print(c(length(genlist), pathway))
...
pdf(file="../plots/fig6_updated_newgenelist_p3_jan24.pdf",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()

svg(file="../plots/fig6_updated_newgenelist_p3_jan24.svg",height = 15, width = 10)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()
...
```{r}
## get target
showdf = read.csv('../data/figure6_show_ - Sheet1.csv')
showdf = subset(showdf, showdf$Show.in.fig. == 'Y')
targets = showdf$Name

df_meta_presaved = df_meta
df_meta_presaved_sub = df_meta_presaved[,colnames(df_meta_presaved) %in% targets]
df_meta_presaved_sub_meta = cbind(df_meta_presaved_sub, df_meta_presaved[,c(1:14)])

write.csv(df_meta_presaved_sub_meta, '../result/df_meta_scores_updated_showVer_jan24.csv')
write.csv(df_meta, '../result/df_meta_scores_updated_FullVer_jan24.csv')
```


################ get treg related stuff
...
targetScore=gsva(t(df),list(reg),method="ssgsea",ssgsea.norm=TRUE,verbose=T)
result['top5_treg_genes'] = t(targetScore)

##### then try 	GSE15659_TREG_VS_TCONV_UP
pathway = 'GSE15659_TREG_VS_TCONV_UP' 

gmt_file_match_im = unlist(gmt_file_imm$genesets[gmt_file_imm$geneset.names ==  pathway]) 
genlist = gmt_file_match_im
targetScore=gsva(t(df),list(genlist),method="ssgsea",ssgsea.norm=TRUE,verbose=T)
result[pathway] = t(targetScore)
...
pdf(file="../plots/fig6_updated_treg.pdf",height = 15, width = 5)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()

svg(file="../plots/fig6_updated_treg.svg",height = 15, width = 5)
pheatmap(plotdf, cluster_rows=FALSE,
         color=colorRampPalette(c("#FF00FF", "#000000", "#FFFF00"))(50),
         breaksList = seq(-0.4, 0.4, by = 0.025))
dev.off()
...
Circo_func <- function(cell_type = "CD4naive",pathways = "T cell dysfunction", condition = "EBV+", save.path = "/bmbl_data/yuzhou/collaborative/Sizun_lab/EBV_project/Circosplot/out/",...){
  ...
  df_p <- as.data.frame(Coef_df.p.filter.isct[diff_coef[order(diff_coef,decreasing = T)] > 0.15,])
  df_n <- as.data.frame(Coef_df.n.filter.isct[diff_coef[order(diff_coef,decreasing = T)] > 0.15,])
  ...
  min_color_val <- round(min(c(df_p$Score,df_n$Score)),1)
  max_color_val <- round(max(c(df_p$Score,df_n$Score)),1)
  ...
  col_fun = colorRamp2(c(min_color_val,0,
                         max_color_val), c("#5a8fbc","white","#d37c6c"),transparency = 0)
  ...
  par(bg = NA)
  circos.par(cell.padding = c(0, 0, 0, 0), track.margin = c(-0.15,0.2),circle.margin = c(0.1,0.8,0.1,0.2))
  ...
  min_color_val_1 <- round(min(c(df_p$Score,df_n$Score)),1)-0.1
