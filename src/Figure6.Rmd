...
prog_t_genes = c('BACH2', 'BCL6', 'CCR7', 'CD28', 'CXCL10', 'CXCR5', 'HIF1A', 'ID3', 'IL7R', 'LEF1', 'SATB1', 'SELL', 'TCF7', 'TNF', 'TNFSF14', 'XCL1')

term_t_genes = c('CCL3', 'CCL4L2', 'CCL5', 'CD38', 'PRGFRN', 'CSF1', 'ENTPD1', 'FASLG', 'GZMA', 'GZMB', 'HAVCR2', 'ID2', 'IFNG', 'IL2RB', 'PDCD1', 'PRDM1', 'PRF1', 'TNFRSF9', 'TNFSF10')
...
targetScore=gsva(t(df),list(prog_t_genes),method="ssgsea",ssgsea.norm=TRUE,verbose=T)
result['prog_tcell'] = t(targetScore)
df_meta['prog_tcell'] = t(targetScore)

targetScore=gsva(t(df),list(term_t_genes),method="ssgsea",ssgsea.norm=TRUE,verbose=T)
result['term_tcell'] = t(targetScore)
df_meta['term_tcell'] = t(targetScore)


df2 = subset(df, df_meta$EBV == 'pos' & df_meta$Tu_ind == 'Tumor')
df_meta2 = subset(df_meta, df_meta$EBV == 'pos' & df_meta$Tu_ind == 'Tumor')


########### hclust only on positive sample - 2 clusters version

ebvglist = c('EBER1', 'EBER2', 'LMP1', 'EBNA2', 'BALF1', 'BCRF1', 'BNLF2A', 'BNLF2B',
             'BZLF1', 'RPMS1')
df2_ebv = df2[,ebvglist]
hc3 <- cutree(hclust(dist(df2_ebv, "euclidean")), 2)
df_meta2$hc = hc3
...
df_meta2$hc = as.factor(df_meta2$hc)

### sample ids
hc2 = df_meta2$ROI_num[df_meta2$hc == 2]
hc1 = df_meta2$ROI_num[df_meta2$hc == 1]

## get only t cell roi mean
df_meta_t = subset(df_meta, df_meta$T_ind == 'T')
df_meta_t %>% group_by(ROI_num) %>%
  summarise(across(.cols = where(is.numeric),.fns = mean)) -> roilev
roilev$group = 'neg'
roilev$group[roilev$ROI_num %in% hc1] = 'ebv++'
roilev$group[roilev$ROI_num %in% hc2] = 'ebv+'

library(ggplot2)
library(ggpubr)

roilev$group = factor(roilev$group, levels = c('neg', 'ebv+', 'ebv++'))
my_comparisons <- list( c("neg", "ebv+"), c("neg", "ebv++"), c("ebv+", "ebv++") )
p = ggplot(roilev, aes(x = group, y = term_tcell, fill = group)) + geom_violin(trim=FALSE)+
  geom_boxplot(width=0.05, fill = 'white')+
  theme_classic() + ggtitle('term Tex, hclust 2') + 
  stat_compare_means(comparisons = my_comparisons, method.args = list(alternative = "less"))
ggsave('../plots/termTex_hclust2_v2.svg', width = 7, height = 5)
p
...
#### Load Seurat Object
```{r}
so = readRDS("cHL.CosMx1k.seuratobject.rds")
```

#### Set colors
```{r}
celltype_color =  c("CD4+ T cells" = "#4DAF4A",
                     "Macrophages" = "#f7c173",
                     "NK" = "#FFFF33",
                     "Fibroblasts"= "green",
                     "DC" = "#377EB8",
...
```{r, message=FALSE}
DimPlot(so, label = T, label.size = 3, reduction = "umap",cols = celltype_color) + NoLegend()
ggsave("Figure6.G.pdf")
```

### Figure 6H

```{r}
cellprops=SCpubr::do_BarPlot(so, 
                              group.by = "Cell.Type",
                              split.by = "EBV",
                              position = "fill",
                              order = T, order.by = "CD8+ T cells",
                              flip = T,
                              add.n = TRUE,
                              add.n.size = 4, colors.use =  celltype_color,
                              return_data = T)


celltype_log2c <-  cellprops$Data %>% group_by(EBV) %>% 
    mutate(total_n = sum(n)) %>%  ungroup() %>% 
    group_by(Cell.Type) %>% 
    mutate(diff = log2((lag(n)/lag(total_n)) / (n/total_n))) %>% drop_na()  %>% 
    ggplot(aes(x = reorder(Cell.Type, diff), y = diff, fill = Cell.Type)) +
    geom_col() + 
    ...
    scale_fill_manual('Cell.Type', values = celltype_color, na.value = 'white') +
    theme_bw() + 
...
ggsave('Figure6.H.pdf', celltype_log2c, dpi = 320, width = 12, height = 6)
```

### GSVA Exhaustion score among CD4 or CD8 cells

```{r, warning=FALSE}
# Exhaustion markers 
markers <-list(c("CTLA4", "EOMES", "HAVCR2", "LAG3", "PDCD1", "TIGIT", "TOX", "PRF1"))
names(markers) <- c("EXHAUSTION")

library("BiocParallel")

# Set the number of cores (use all but one)
ncores <- max(1, parallel::detectCores() - 1)
bp_param <- MulticoreParam(workers = ncores)


# Add GSVA exhaustion score measured specifically among CD4 T cells
# subset only CD4+
soCD4 = subset(so, Cell.Type %like% "CD4")

# Conversion to SingleCellExperiment format
sce <- as.SingleCellExperiment(DietSeurat(soCD4))

# Extract only LMP1 for the Expression column
lmp1_expr <- data.frame(
  Expression = as.matrix(assay(sce, "counts"))["LMP1", ],
  row.names = colnames(sce)
)
soCD4@meta.data <- cbind(soCD4@meta.data, lmp1_expr)

# GSVA score across the entire matrix in parallel
expr_all <- as.matrix(assay(sce, "counts"))
gsva_results <- GSVA::gsva(expr_all, markers, verbose = TRUE, BPPARAM = bp_param)
gsva_results <- t(gsva_results)
gc()

# Update metadata with GSVA scores
soCD4@meta.data[rownames(gsva_results), colnames(gsva_results)] <- gsva_results

soCD4$EXHamongCD4=soCD4$EXHAUSTION
soCD4$EXHAUSTION=NULL

so=AddMetaData(so, metadata = soCD4@meta.data, col.name = "EXHamongCD4")

# subset only CD8+
soCD8 = subset(so, Cell.Type %like% "CD8")

# Conversion to SingleCellExperiment format
sce <- as.SingleCellExperiment(DietSeurat(soCD8))
...
soCD8@meta.data <- cbind(soCD8@meta.data, lmp1_expr)
...
# Update metadata with GSVA scores
soCD8@meta.data[rownames(gsva_results), colnames(gsva_results)] <- gsva_results
soCD8$EXHamongCD8=soCD8$EXHAUSTION
soCD8$EXHAUSTION=NULL

so=AddMetaData(so, metadata = soCD8@meta.data, col.name = "EXHamongCD8")
```
...
p = ggplot(soCD4@meta.data, aes(x=EBV, y=EXHamongCD4, fill=Tumor)) + 
    geom_violin(trim = FALSE) + geom_boxplot(width=0.4, position=position_dodge(0.9)) + theme_bw() + scale_fill_manual(values=c("#f6adff", "#e1d936"))  
p + stat_compare_means(method = "wilcox.test", method.args = list(alternative="greater"), label="p.signif") + xlab("") + ylab("GSVA Exhaustion Score") 

ggsave("Figure6.I.CD4.pdf")


soCD8=subset(so, Cell.Type %like% "CD8")

p = ggplot(soCD8@meta.data, aes(x=EBV, y=EXHamongCD8, fill=Tumor)) + 
    geom_violin(trim = FALSE) + geom_boxplot(width=0.4, position=position_dodge(0.9)) + theme_bw() + scale_fill_manual(values=c("#f6adff", "#e1d936"))  
p + stat_compare_means(method = "wilcox.test", method.args = list(alternative="greater"), label="p.signif") + xlab("") + ylab("GSVA Exhaustion Score") 

ggsave("Figure6.I.CD8.pdf")
```


### GSVA Exhaustion score in EBV+
```{r, eval=T}
library(imcRtools)

# subset only EBV+
so = subset(so, EBV == "EBV+")

# Conversion to SingleCellExperiment format
sce <- as.SingleCellExperiment(DietSeurat(so))
...
so@meta.data <- cbind(so@meta.data, lmp1_expr)


# add LMP1 pseudofov expression when higher or lower than mean
so$LMP1fov_expr <- "Low"
so$LMP1fov_expr[so$fov=="8_TMA_98.3"]<-"High"
so$LMP1fov_expr[so$fov=="6_TMA_98.3"]<-"High"
so$LMP1fov_expr[so$fov=="6_HTMA_528"]<-"High"
so$LMP1fov_expr[so$fov=="5_HTMA_528"]<-"High"
so$LMP1fov_expr[so$fov=="3_TMA_98.3"]<-"High"
so$LMP1fov_expr[so$fov=="3_HTMA_528"]<-"High"
so$LMP1fov_expr[so$fov=="25_TMA_98.3"]<-"High"
so$LMP1fov_expr[so$fov=="22_TMA_98.3"]<-"High"
so$LMP1fov_expr[so$fov=="20_HTMA_528"]<-"High"
so$LMP1fov_expr[so$fov=="2_TMA_98.3"]<-"High"
so$LMP1fov_expr[so$fov=="2_HTMA_528"]<-"High"
so$LMP1fov_expr[so$fov=="19_HTMA_528"]<-"High"
so$LMP1fov_expr[so$fov=="18_HTMA_528"]<-"High"
...
# Update metadata with GSVA scores
so@meta.data[rownames(gsva_results), colnames(gsva_results)] <- gsva_results
```


### Calculate distance to tumor cell
```{r, eval=T}
library(imcRtools)

soExpTum=subset(so@meta.data, Cell.Type=="Tumor")

# Calculation of the 87th percentile threshold
thr <- quantile(soExpTum$Expression, 0.87)

# Direct identification of cell IDs with High/Low LMP1 expression
TumLow  <- rownames(soExpTum[soExpTum$Expression <  thr, ])
TumHigh <- rownames(soExpTum[soExpTum$Expression >= thr, ])

# Add cell ID to metadata
so@meta.data$cell_ID <- rownames(so@meta.data)

# Helper function
process_group <- function(so_obj, exclude_ids, label) {
  sce <- as.SingleCellExperiment(DietSeurat(subset(so_obj, cell_ID %in% exclude_ids, invert = TRUE)))
  EBV <- minDistToCells(
    sce,
    x_cells = sce$Cell.Type == "Tumor",
    coords = c("centroid_x", "centroid_y"),
    img_id = "fov"
  )
  seurat_obj <- as.Seurat(EBV, counts = "counts", data = NULL)
  df <- seurat_obj@meta.data
  df$LMP1expr_NearestTumorCell <- label
  rm(sce, EBV, seurat_obj)
  gc()
  df
}

# Two-step processing calculating distance vs High and Low LMP1 tumor cells
so@meta.data <- rbind(
  process_group(so, TumLow,  "High"),
  process_group(so, TumHigh, "Low")
  ) 

so@meta.data$distToTumorCell = so@meta.data$distToCells
so@meta.data$distToCells = NULL
so@meta.data=subset(so@meta.data, LMP1fov_expr == LMP1expr_NearestTumorCell)
```
...
```{r}
# subset only EBV+
cHL = subset(so@meta.data, EBV == "EBV+")

ggplot(subset(cHL, Cell.Type %like% "CD8" ), aes(x=distToTumorCell, y=EXHAUSTION)) + 
    scale_colour_manual(values=c("#9e3135ff", "#e6ac5aff")) +
    geom_smooth(method=lm, mapping = aes(color = LMP1expr_NearestTumorCell)) + stat_cor(method = "spearman",mapping = aes(color = LMP1expr_NearestTumorCell)) + theme_bw() + scale_x_continuous(name="Distance of CD8+ T-cells to Tumor")+ scale_y_continuous(name="GSVA score of T-cell exhaustion") + guides(color=guide_legend(title="LMP1 expression")) 

ggsave("Figure6.K.CD8.pdf", width = 9, height = 6)


ggplot(subset(cHL, Cell.Type %like% "CD4" ), aes(x=distToTumorCell, y=EXHAUSTION)) + 
    scale_colour_manual(values=c("#9e3135ff", "#e6ac5aff")) +
    geom_smooth(method=lm, mapping = aes(color = LMP1expr_NearestTumorCell)) + stat_cor(method = "spearman",mapping = aes(color = LMP1expr_NearestTumorCell)) + theme_bw() + scale_x_continuous(name="Distance of CD4+ T-cells to Tumor")+ scale_y_continuous(name="GSVA score of T-cell exhaustion") + guides(color=guide_legend(title="LMP1 expression")) 

ggsave("Figure6.K.CD4.pdf", width = 9, height = 6)
```

### Figure 6J

```{r}
#select only T cells and keep the T cell with the minimum distance to Tumor cell
cHLT=subset(cHL, Cell.Type %like% "CD")

cHLT = cHLT %>%
  group_by(row.names(cHLT)) %>%
  slice_min(distToTumorCell) %>%
  ungroup

p = ggplot(subset(cHLT, Cell.Type %like% "CD8"), aes(x=LMP1expr_NearestTumorCell, y=EXHAUSTION, fill=LMP1expr_NearestTumorCell)) + 
geom_violin(trim = FALSE) + geom_boxplot(width=0.4, position=position_dodge(0.9)) + theme_bw() + scale_fill_manual(values=c("#9e3135ff", "#e6ac5aff")) 

p + stat_compare_means(method = "wilcox.test", method.args = list(alternative="greater"), label="p.signif") + xlab("LMP1 expression group") + ylab("GSVA score of CD8 T-cell exhaustion") 

ggsave("Figure6.PanelJ.CD8.pdf", width = 6, height = 6)


p = ggplot(subset(cHLT, Cell.Type %like% "CD4"), aes(x=LMP1expr_NearestTumorCell, y=EXHAUSTION, fill=LMP1expr_NearestTumorCell)) + 
geom_violin(trim = FALSE) + geom_boxplot(width=0.4, position=position_dodge(0.9)) + theme_bw() + scale_fill_manual(values=c("#9e3135ff", "#e6ac5aff")) 

p + stat_compare_means(method = "wilcox.test", method.args = list(alternative="greater"), label="p.signif") + xlab("LMP1 expression group") + ylab("GSVA score of CD4 T-cell exhaustion")

ggsave("Figure6.PanelJ.CD4.pdf", width = 6, height = 6)
```
